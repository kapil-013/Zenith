    ...
    <div className="flex justify-center items-center min-h-[70vh] px-4">
      <NeumorphicCard className="w-full max-w-md p-8 animate-in fade-in zoom-in-95 shadow-[var(--shadow-neumorphic-floating)] border border-transparent">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-[var(--color-civic-text-primary)] tracking-tight">{t("Forgot_Title")}</h1>
        ...
        {!sent ? (
          <form onSubmit={handleReset} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-[var(--color-civic-text-secondary)] mb-2">{t("Forgot_Email")}</label>
              <NeumorphicInput
                type="email"
                value={email}
            ...
            <NeumorphicButton type="submit" className="w-full justify-center font-bold" variant="primary" disabled={loading}>
              {loading ? t("Forgot_ButtonLoading") : t("Forgot_Button")}
            </NeumorphicButton>
          </form>
        ) : (
