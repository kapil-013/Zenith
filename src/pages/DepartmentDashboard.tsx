...
export function DepartmentDashboard() {
    ...
    const fetchAssigned = async () => {
      ...
      try {
        ...
        const q1 = query(
          ...
          where("assignedTo", "==", departmentName),
        );
        const q2 = query(
          collection(db, "issues"),
          where("suggestedDepartment", "==", departmentName),
        );

        const [snap1, snap2] = await Promise.all([getDocs(q1), getDocs(q2)]);
        const combined = new Map<string, Issue>();
        snap1.forEach((d) =>
          combined.set(d.id, { id: d.id, ...d.data() } as Issue),
        );
        snap2.forEach((d) =>
          combined.set(d.id, { id: d.id, ...d.data() } as Issue),
        );
  ...
  const stats = {
    ...
    inProgress: issues.filter((i) =>
      inProgressStatuses.includes(i.status || i.currentStatus),
    ).length,
    ...
    <div className="space-y-6">
      <div className="mb-8">
        ...
        <p className="text-[var(--color-civic-text-secondary)] font-medium mt-2">
          Manage and update issues assigned to your department.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            color: "text-[var(--color-civic-status-inprogress)]",
          },
          {
            label: "Resolved",
          ...
          },
          {
            label: "Urgent",
            value: stats.urgent,
            color: "text-[var(--color-civic-danger)]",
          },
        ].map((stat, i) => (
      ...
      <h2 className="text-xl font-extrabold text-[var(--color-civic-text-primary)] mt-8 mb-4 tracking-tight">
        Assigned Issues
      </h2>

      {loading ? (
        <div className="text-center py-8 text-[var(--color-civic-text-secondary)] font-medium">
          Loading...
        </div>
      ) : issues.length === 0 ? (
